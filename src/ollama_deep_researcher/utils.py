...
def deduplicate_and_format_sources(
    ...
    # Handle empty results case - return meaningful message instead of empty
...
def format_sources(search_results: Dict[str, Any]) -> str:
    ...
    Enhanced to show source paths for local files.
...
def query_local_vector_store(query: str, vector_store_paths: List[str], embedding_model: str, limit: int = 5) -> Dict[str, Any]:
    ...
    Now enhanced with FET sensor RAG support for hierarchical retrieval.
    ...
    """
    try:
        # Check if we have FET sensor data paths
        excel_vectorstore_path = None
        non_excel_vectorstore_path = None
        publication_paths = []

        for path in vector_store_paths:
            if "excel_vectorstore" in path or "excel_raw_data_vectorization/excel_vectorstore" in path:
                excel_vectorstore_path = path
                logger.info(f"Detected Excel FET vectorstore: {path}")
            elif "non_excel_vectorstore" in path and "excel" not in path:
                non_excel_vectorstore_path = path
                logger.info(f"Detected non-Excel vectorstore: {path}")
            else:
                publication_paths.append(path)
                logger.info(f"Detected publication vectorstore: {path}")

        # If we have FET sensor data, use advanced RAG
        if excel_vectorstore_path:
            logger.info("Using advanced FET sensor RAG system")
            try:
                from ollama_deep_researcher.advanced_fet_rag import FETSensorRAG

                # Initialize FET RAG with available paths
                # Pass config for LLM if available
                config = None
                if 'config' in locals():
                    config = locals()['config']

                fet_rag = FETSensorRAG(
                    excel_vectorstore_path=excel_vectorstore_path,
                    non_excel_vectorstore_path=non_excel_vectorstore_path,
                    embedding_model=embedding_model,
                    k_folders=8,           # Top folders to retrieve
                    k_runs_per_folder=4,   # Runs per folder
                    k_non_excel=3,         # Context documents
                    config=config          # Pass config for LLM initialization
                )

                # Query with advanced hierarchical retrieval
                logger.info(f"Executing FET RAG query: {query}")
                fet_results = fet_rag.query(query)

                # If we also have publication paths, merge results
                if publication_paths:
                    logger.info("Merging FET results with publication search")
                    pub_results = _query_simple_stores(query, publication_paths, embedding_model, limit)

                    # Combine results, prioritizing FET data
                    combined_results = fet_results.get("results", [])
                    combined_results.extend(pub_results.get("results", []))

                    return {
                        "results": combined_results[:limit * 2],  # Return more results when combining
                        "source": "multi_modal_rag",
                        "stats": {
                            **fet_results.get("stats", {}),
                            "publication_results": len(pub_results.get("results", []))
                        }
                    }
                ...
                logger.warning(f"Failed to import FETSensorRAG: {e}. Falling back to simple RAG.")
            except Exception as e:
                logger.error(f"Error using FET RAG: {e}. Falling back to simple RAG.")

        # Fall back to original simple RAG implementation
        logger.info("Using simple RAG (no FET data detected or FET RAG failed)")
        return _query_simple_stores(query, vector_store_paths, embedding_model, limit)

    except Exception as e:
        logger.error(f"Error in query_local_vector_store: {str(e)}")
        import traceback
        logger.error(traceback.format_exc())
        return {"results": [], "source": "local_vector_store", "error": str(e)}

def _query_simple_stores(query: str, vector_store_paths: List[str], embedding_model: str, limit: int = 5) -> Dict[str, Any]:
    """
    Original simple RAG implementation for publication/document stores.
    Extracted as a helper function for backward compatibility.
    """
    try:
        # Auto-adjust file descriptor limit for large-scale retrieval
        if len(vector_store_paths) > 50:
            try:
                import resource
                soft, hard = resource.getrlimit(resource.RLIMIT_NOFILE)
                # Need at least 5 file descriptors per vector store plus overhead
                needed = len(vector_store_paths) * 5 + 100
                if soft < needed:
                    new_limit = min(needed, hard)
                    resource.setrlimit(resource.RLIMIT_NOFILE, (new_limit, hard))
                    logger.info(f"Adjusted file descriptor limit from {soft} to {new_limit} for {len(vector_store_paths)} vector stores")
            except Exception as e:
                logger.warning(f"Could not adjust file descriptor limit: {e}. May need manual ulimit setting.")

        # Initialize embedding model
        device = _get_best_available_device()
        logger.info(f"Using device: {device} for embeddings in simple RAG")
        ...
        batch_size = 10  # Process 10 vector stores at a time (reduced from 20 for macOS stability)
        for i in range(0, len(vector_store_paths), batch_size):
            ...
            for store_path in batch_paths:
                # Initialize variables for cleanup tracking - MUST be before any conditionals
                client = None
                db = None
                ...
                if os.path.exists(store_path):
                    ...
                    if 'chroma.sqlite3' in chroma_files:
                        ...
                        try:
                            # Try to load the vector store with compatibility handling
                            try:
                                # First try with langchain-chroma (new version)
                                from langchain_chroma import Chroma as ChromaNew
                                db = ChromaNew(
                                    persist_directory=store_path,
                                    embedding_function=embeddings
                                )
                                logger.info("Using langchain-chroma (new version)")
                            except (ImportError, KeyError) as e:
                                # Fallback to langchain-community version
                                if "_type" in str(e):
                                    logger.warning(f"Chroma version compatibility issue: {e}")
                                    logger.info("Attempting compatibility fix...")

                                    # Try to create a new collection with same embeddings
                                    # This works around the _type error
                                    client = None  # Track client for cleanup
                                    try:
                                        import chromadb
                                        client = chromadb.PersistentClient(path=store_path)

                                        # Get or create collection with explicit configuration
                                        ...
                                        logger.info("Successfully created Chroma wrapper with compatibility fix")
                                    except Exception as fix_error:
                                        logger.error(f"Compatibility fix failed: {fix_error}")
                                        # Clean up client if created
                                        if client is not None:
                                            try:
                                                # Close the client to release file descriptors
                                                if hasattr(client, 'close'):
                                                    client.close()
                                                del client
                                            except Exception as close_error:
                                                logger.warning(f"Error closing client: {close_error}")
                                        # Last resort: skip this store
                                        logger.warning(f"Skipping incompatible vector store: {store_path}")
                                        continue
                                else:
                                    ...
                                    logger.info("Using langchain-community Chroma")
                            ...
                            try:
                                ...
                                # This is crucial for handling 500+ vector stores
                                try:
                                    # Clean up references but DO NOT call _system.stop()
                                    # ChromaDB has a known file descriptor leak issue but calling stop()
                                    # breaks the global system state for all subsequent connections
                                    if db is not None:
                                        try:
                                            # Clear internal references only
                                            if hasattr(db, '_client'):
                                                db._client = None
                                            if hasattr(db, '_collection'):
                                                db._collection = None
                                        except Exception as e:
                                            logger.debug(f"Error clearing Chroma wrapper internals: {e}")
                                        finally:
                                            del db
                                            db = None

                                    # Clear standalone client reference without stopping
                                    if client is not None:
                                        del client
                                        client = None

                                    # Force garbage collection to release resources
                                    # This is where ChromaDB should properly close files
                                    import gc
                                    gc.collect()

                                except Exception as cleanup_error:
                                    logger.warning(f"Error during cleanup for {store_path}: {cleanup_error}")
        ...
        results = []
        logger.debug(f"--- Formatting Top {len(top_docs)} Docs ---") # DEBUG
        for i, (doc, score) in enumerate(top_docs):
            logger.debug(f"Processing doc {i+1}/{len(top_docs)}, Score: {score}") # DEBUG
            # Extract metadata
            metadata = doc.metadata
            logger.debug(f"  Metadata keys: {list(metadata.keys())}") # DEBUG
            # Log first few metadata values for inspection
            for k, v in list(metadata.items())[:5]: # Log first 5 metadata items
                 logger.debug(f"    Metadata '{k}': {str(v)[:100]}...") # DEBUG
            
            page_content_len = len(doc.page_content)
            logger.debug(f"  Page content length: {page_content_len}") # DEBUG
            # logger.debug(f"  Page content start: {doc.page_content[:100]}...") # DEBUG (Optional: uncomment if needed)

            # --- Merged Metadata Extraction (Revised Logic) ---

            # 1. Primary ID (Combine attempts, prioritize specific IDs over Unknown)
            potential_ids = [
            ...
            primary_id = next((pid for pid in potential_ids if pid and pid != 'Unknown'), 'Unknown')
            logger.debug(f"  Derived primary_id: {primary_id}") # DEBUG

            # 2. Title (Prefer explicit 'title', fallback to specific types, then generic)
            title = metadata.get('title') # Try explicit title first
            logger.debug(f"  Initial title from metadata: {title}") # DEBUG
            if not title:
                # Try Material Science style if paper_id exists
                paper_id_val = metadata.get('paper_id')
                if paper_id_val and paper_id_val != 'Unknown':
                     title = f"Scientific Paper: {paper_id_val}"
                     logger.debug(f"  Using Scientific Paper title format.") # DEBUG
                else:
                     # Fallback to generic local document title using best available ID
                     title = f"Local Document: {primary_id}"
                     logger.debug(f"  Using generic Local Document title format.") # DEBUG
            else:
                 logger.debug(f"  Using title found directly in metadata.") # DEBUG

            # 3. URL and Source Path (Use ML logic - more robust fallbacks)
            url = metadata.get('url')
            source_path = metadata.get('source', metadata.get('source_path', metadata.get('source_file')))
            logger.debug(f"  Initial url from metadata: {url}") # DEBUG
            logger.debug(f"  Source path from metadata: {source_path}") # DEBUG

            if not url:
                doi = metadata.get('doi')
                if doi and isinstance(doi, str) and doi != 'Unknown':
                    url = f"https://doi.org/{doi}"
                    logger.debug(f"  Constructed DOI url: {url}") # DEBUG
                elif source_path and source_path != 'Unknown':
                    # Use actual file path if available
                    url = f"file://{source_path}"
                    logger.debug(f"  Constructed file url from source_path: {url}") # DEBUG
                else:
                    ...
                    if source_file and line_num:
                        url = f"local://{os.path.basename(source_file)}#L{line_num}"
                        logger.debug(f"  Constructed local file url: {url}") # DEBUG
                    else:
                        url = f"local://{primary_id}"
                        logger.debug(f"  Using fallback local ID url: {url}") # DEBUG
            # --- End of Merged Extraction ---

            # Add source path info to the content if available
            content_with_source = doc.page_content
            if source_path and source_path != 'Unknown':
                content_with_source = f"[Source Path: {source_path}]\n\n{doc.page_content}"

            # Create result structure
            result = {
                "title": title,
                "url": url,
                "content": content_with_source[:500] + "...",
                "raw_content": content_with_source,
                "metadata": {
                    **metadata,
                    "source_path": source_path if source_path else primary_id  # Ensure source_path is in metadata
                },
                "score": score
            }
            results.append(result)
            logger.info(f"  Formatted result: title='{result['title']}', url='{result['url']}', content_len={len(result['content'])} raw_len={len(result['raw_content'])}") # DEBUG
        ...
        logger.info(f"Returning {len(results)} formatted results from simple RAG")
        # logger.debug(f"Formatted results structure: {formatted_results}") # DEBUG (Optional: very verbose)
...
def _get_best_available_device() -> str:
    ...
    try:
        ...
        # AttributeError: torch version doesn't have expected attributes
        # Exception: any other unexpected errors
        logger.debug(f"Error detecting compute device, defaulting to CPU: {str(e)}")
        return "cpu"
