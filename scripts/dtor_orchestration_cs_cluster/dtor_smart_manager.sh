#!/bin/bash
# Smart DToR Manager - Automatically detects current stage and submits appropriate jobs
# Usage: bash dtor_smart_manager.sh <topic_file> [chain_count]
...
if [ ! -f "$PERSPECTIVES_FILE" ]; then
    ...
    if [ $(check_job_running "DToR_Div_${JOB_SUFFIX}") -gt 0 ]; then
        ...
        JOB_ID=$(sbatch --parsable --job-name="DToR_Div_${JOB_SUFFIX}" dtor_orchestration/dtor_stage1_diversify.sh "$TOPIC_FILE")
...
if [ -n "$NEED_SUBMISSION" ]; then
    ...
    for BRANCH_ID in $NEED_SUBMISSION; do
        ...
        JOB_ID1=$(sbatch --parsable --job-name="DToR_Br_${JOB_SUFFIX}" dtor_orchestration/dtor_stage2_branch.sh "$TOPIC_FILE" "$BRANCH_ID")
        ...
        for i in $(seq 2 $CHAIN_COUNT); do
            JOB_ID=$(sbatch --parsable --job-name="DToR_Br_${JOB_SUFFIX}" --dependency=afterany:$PREV_JOB dtor_orchestration/dtor_stage2_branch.sh "$TOPIC_FILE" "$BRANCH_ID")
...
JOB_ID=$(sbatch --parsable --job-name="DToR_Fin_${JOB_SUFFIX}" dtor_orchestration/dtor_stage3_finalize.sh "$TOPIC_FILE")
